...
import useConversation from "../Zustand/useConversation"; // Zustand store import karein
...
function HomePage() {
  ...
  return (
    <Home protect={true}>
      ...
      */}
      <div className={`${selectedConversation ? "hidden" : "flex"} md:flex h-full w-full md:w-auto border-r border-[#313d45]/50`}>
        <Sidebar />
      </div>
      ...
      */}
      <div className={`${!selectedConversation ? "hidden" : "flex"} md:flex flex-1 h-full`}>
        <MessageContainer />
      </div>
    </Home>
  );
